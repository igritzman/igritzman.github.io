...
function airportPromptTarget(region: (typeof regions)[number], airport: string) {
  const airportCityHints: Record<string, string> = {
    PVG: "Shanghai",
    SHA: "Shanghai",
    PEK: "Beijing",
    PKX: "Beijing",
      ...
      const monthlyQuestion: Question =
        template === 0 ? {
          ...
          prompt: primaryAirport.startsWith("No ")
            ? `Which airport note belongs to ${region.name}?`
            : `Identify a major airport clue for ${airportPromptTarget(region, primaryAirport)}.`,
          ...
          difficulty,
          inputType: "multiple-choice",
          prompt: `Which urban transit clue belongs to ${region.name}?`,
          answer: primaryMetro,
          ...
          difficulty,
          inputType: "multiple-choice",
          prompt: `Which maritime, river, ferry, or port clue fits ${region.name}?`,
          ...
          difficulty,
          inputType: "multiple-choice",
          prompt: `${primaryHighway} is a road or traffic corridor clue for which country?`,
          answer: region.name,
          choices: choicesFor(region.name, countryPool, seed),
